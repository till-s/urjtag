...
static PyObject *
urj_pyc_signal(urj_pychain_t *self, int in, PyObject *args)
{
    urj_chain_t  *urc = self->urchain;
    urj_part_t   *part;
    urj_part_signal_t *s;
    const char   *signam = NULL;
    long          val    = -1;
    int           dir_out;

    if ( in ) {
        if (!PyArg_ParseTuple(args, "s|k", &signam, &val))
            return NULL;
    } else {
        if (!PyArg_ParseTuple(args, "s", &signam))
            return NULL;
    }
    ...
    s = urj_part_find_signal (part, signam);
    if (!s)
    {
    ...
    if ( in )
    {
        if ( ! (dir_out = (val >= 0 )) )
            val = 0;
        return urj_py_chkret (urj_part_set_signal (part, s, dir_out, val));
    }
    else
    {
        val = urj_part_get_signal (part, s);
        if ( val < 0 )
            return urj_py_chkret ( -1 );
...
{
    return urj_pyc_signal (self, 1, args);
}

static PyObject *
urj_pyc_get_signal(urj_pychain_t *self, PyObject *args)
{
    return urj_pyc_signal (self, 0, args);
}
...
static PyMethodDef urj_pyc_methods[] =
{
    ...
    {"set_signal", (PyCFunction)urj_pyc_set_signal, METH_VARARGS,
     "set_signal SIG [0|1]; set BSR signal value and direction ('in' if no val)"},
    {"get_signal", (PyCFunction)urj_pyc_get_signal, METH_VARARGS,
     "get BSR signal value"},
